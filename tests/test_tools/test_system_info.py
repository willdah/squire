...
async def test_system_info_basic(mock_backend, mock_registry):
    ...
    mock_backend.set_response("df", CommandResult(
        returncode=0, stdout="/dev/disk1s1  500G  200G  300G  40%  /\n", stderr="",
    ))
    mock_backend.set_response("uptime", CommandResult(
        returncode=0, stdout="up 5 days\n", stderr="",
    ))
...
async def test_system_info_remote_host(mock_backend, mock_registry):
    ...
    mock_backend.set_response("hostname", CommandResult(
        returncode=0, stdout="media-server\n", stderr="",
    ))
    mock_backend.set_response("uname", CommandResult(
        returncode=0, stdout="Linux 6.1.0\n", stderr="",
    ))
    mock_backend.set_response("nproc", CommandResult(
        returncode=0, stdout="4\n", stderr="",
    ))
    mock_backend.set_response("grep", CommandResult(
        returncode=0, stdout="cpu  1000 200 300 500 100 50 20 0 0 0\n", stderr="",
    ))
    free_stdout = "              total        used        free\nMem:          16384        8192        8192\n"
    mock_backend.set_response("free", CommandResult(
        returncode=0, stdout=free_stdout, stderr="",
    ))
    mock_backend.set_response("df", CommandResult(
        returncode=0, stdout="/dev/sda1  100G  50G  50G  50%  /\n", stderr="",
    ))
    mock_backend.set_response("uptime", CommandResult(
        returncode=0, stdout="up 30 days\n", stderr="",
    ))
